    ...
    (if (empty? lst1)
        (cont lst2)
         (append$ (cdr lst1) lst2
           (lambda(lst1-cdr-res)
             (cont(cons (car lst1) lst1-cdr-res )))))))
         ...
         (equal-trees$ (car tree1) (car tree2)
                       (lambda (car-res)
                         (equal-trees$ (cdr tree1) (cdr tree2)
                                       (lambda (cdr-res)
                                         (succ (cons car-res cdr-res)))fail)
                         )fail)))
  )
)
...
(define as-real
  (lambda (x)
    (letrec ((lazy-list (cons-lzl x (lambda () lazy-list))))
      lazy-list)))
...
(define sqrt-with
  (lambda (x y)
    (cons-lzl y
           (lambda ()
                (sqrt-with (tail x) (giveMeNextApp x y))))))
